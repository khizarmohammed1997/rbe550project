...
class gopher():
    ...
    def run(self):
        ...
        while i<3:
            ...
            self.VO_traj.engine(local_goal)
            rospy.sleep(1)
            i=i+1
...
class VOTrajectory:
    ...
    def engine(self,goal):
        ...
        print("amcl_x",self.start.x)
        print("amcl_y",self.start.y)
        while not self.found_path or (not rospy.is_shutdown()):    
            best_step = None
            ...
            self.Robot_vel.linear.x = self.best_vel
            self.Robot_vel.angular.z = -(self.best_theta - self.gopher_amcl_yaw)/5
            # print("VO",self.best_vel)
